...
import { useGetAllTrips, useTripDelete } from '../../api-queries/Trip';
...
function TripTip() {
  ...
  const trips = useGetAllTrips({ limit, page, search });
  const useDelete = useTripDelete();
  ...
  if (trips.isError) return <Error error={trips.error.message} />

  return (
    <>
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className='shadow-xl rounded-lg bg-white overflow-hidden'>
        <div className="p-5 flex items-center justify-between">
          <div className="text-semibold text-xl">Trip-tips</div>
          <div className='flex items-center'>
            <div className='mr-3'>
              <Search value={search} onChange={(e) => (setSearch(e.target.value), setPage(0))} />
            </div>
            <Link to={import.meta.env.VITE_API + '/Trip-tips/add'} className="codes_action_add border rounded-lg py-2 px-4 text-gray-500 flex items-center hover:bg-gray-100 active:bg-gray-200">
        ...
        <div className="codes_table overflow-auto">
          <table className='table-auto w-full'>
            ...
            <tbody>
              {
                trips.data?.data.data.map((tip: any, index: number) => (
                  <tr key={index} className='hover:bg-gray-100 active:bg-gray-200'>
                    {
                      Object.values(cols).map((col, index2: number) => (
                        <td onClick={() => col.id !== 'actions' && navigate(import.meta.env.VITE_API + '/Trip-tips/edit/' + tip.id)} className={'p-3  border-b text-sm ' + ('text-' + col.side) + (col.id !== 'actions' && ' cursor-pointer')} key={index2}>
                          {
          ...
          <Pagination count={trips.data?.data?.count} setPage={setPage} page={page} limit={limit} />
        </div>
      </motion.div>
      <PopupDelete
        open={open}
