...
function Location1() {
  ...
  const useData = useGetLocation(Number(1));
  const useUpdate = useLocationUpdate(Number(1));
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
      <Box>
        <h1 className='font-semibold text-2xl mb-5'>Yangykala Canyons</h1>
        <form onSubmit={sendData} className='grid grid-cols-12 gap-5'>
          ...
          {/* <Textarea className='col-span-8' height={200} label='Description' placeholder="Description" name='description' value={data.description} onChange={handleData} /> */}

          <Input className='col-span-4' label='Meta name' placeholder="Meta name" name='meta_name' required={false} value={data.meta_name} onChange={handleData} />
          <Input className='col-span-4' label='Meta description' placeholder="Meta description" name='meta_description' required={false} value={data.meta_description} onChange={handleData} />
          <Input className='col-span-4' label='Meta keywords' placeholder="Keyword1, keyword2, keyword3, ..." name='meta_keyword' required={false} value={data.meta_keyword} onChange={handleData} />
